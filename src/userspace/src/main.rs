...
use alloc::vec::Vec;
use alloc::vec; // POPRAWKA 1: import makra vec!
...
pub unsafe fn syscall1(num: Syscall, arg1: usize) -> usize {
    let ret: usize;
    core::arch::asm!("int 0x80", in("rax") num as usize, in("rdi") arg1, lateout("rax") ret, options(nostack));
    ret
}
...
pub unsafe fn fast_memcpy(dst: *mut u8, src: *const u8, n: usize) {
    core::arch::asm!(
        ...
        n = in(reg) n, in("rdi") dst, in("rsi") src, out("rcx") _, options(nostack)
    );
}
...
pub unsafe fn fast_memset(dst: *mut u8, val: u8, n: usize) {
    ...
    core::arch::asm!(
        ...
        n = in(reg) n, in("rdi") dst, in("rax") wide, out("rcx") _, options(nostack)
    );
}
...
pub unsafe fn fast_strlen(s: *const u8) -> usize {
    ...
    core::arch::asm!(
        ...
        "not rcx", "dec rcx",
        in("rdi") s, out("rcx") len, out("al") _, options(nostack)
    );
    len
}
...
fn panic(info: &PanicInfo) -> ! {
    ...
    if let Some(location) = info.location() {
        print("  File: "); print(location.file()); print("\n");
    }
    exit(1);
}
...
impl Write for Writer {
    fn write_str(&mut self, s: &str) -> fmt::Result { print(s); Ok(()) }
}
...
macro_rules! println_fmt {
    () => { $crate::print("\n") };
    ($($arg:tt)*) => {{ $crate::print_fmt!($($arg)*); $crate::print("\n"); }};
}
...
impl<K: Hash + PartialEq + Clone, V> HashMap<K, V> {
    ...
    pub fn remove(&mut self, key: &K) -> Option<V> {
        ...
        let mut j = 0usize;
        loop {
            // POPRAWKA 2: borrow przez referencję żeby uniknąć move
            match &self.slots[i] {
                None => return None,
                Some((k, _, false)) if k == key => {
...
pub mod math {
    ...
    #[inline(always)] pub fn popcount(x: u64) -> u32 {
        let r: u64;
        unsafe { core::arch::asm!("popcnt {r}, {x}", r = out(reg) r, x = in(reg) x, options(nostack, nomem)); }
        r as u32
    }
    #[inline(always)] pub fn leading_zeros(x: u64) -> u32 {
        if x == 0 { return 64; }
        let r: u64;
...
pub struct Framebuffer { pub width: usize, pub height: usize, pub buffer: Vec<u32> }
...
impl Framebuffer {
    ...
    pub fn clear(&mut self, color: Color) { self.buffer.fill(color.to_u32()); }
    pub fn draw_rect(&mut self, x: usize, y: usize, w: usize, h: usize, color: Color) {
        ...
        for row in y..y_end {
            ...
            if let Some(slice) = self.buffer.get_mut(base + x..base + x_end) { slice.fill(pixel); }
        }
    }
...
impl Button {
    pub fn new(rect: Rect, label: &str) -> Self {
        Self { rect, label: String::from(label), bg_color: Color::rgb(100,100,200), on_click: None }
    }
...
impl Widget for Button {
    fn bounds(&self) -> Rect { Rect::new(self.rect.x, self.rect.y, self.rect.width, self.rect.height) }
    fn draw(&self, fb: &mut Framebuffer) {
        fb.draw_rect(self.rect.x as usize, self.rect.y as usize,
                     self.rect.width as usize, self.rect.height as usize, self.bg_color);
    }
    fn handle_click(&mut self, x: i32, y: i32) -> bool {
...
pub struct Window { pub title: String, pub rect: Rect, pub widgets: Vec<Box<dyn Widget>> }
...
impl Window {
    ...
    pub fn draw(&self, fb: &mut Framebuffer) {
        fb.draw_rect(self.rect.x as usize, self.rect.y as usize,
                     self.rect.width as usize, self.rect.height as usize, Color::rgb(200,200,200));
        fb.draw_rect(self.rect.x as usize, self.rect.y as usize, self.rect.width as usize, 30, Color::rgb(50,50,150));
...
pub struct Terminal {
    pub width:    usize,
    pub height:   usize,
    // POPRAWKA 3: używamy Vec<Vec<char>> z jawnym vec! przez alloc::vec
...
impl Terminal {
    ...
    pub fn write_str(&mut self, s: &str) { for c in s.chars() { self.write_char(c); } }
    fn scroll(&mut self) {
...
pub struct DriverManager { drivers: Vec<Box<dyn Driver>> }

impl DriverManager {
    pub fn new() -> Self { Self { drivers: Vec::new() } }
    pub fn register(&mut self, driver: Box<dyn Driver>) { self.drivers.push(driver); }
...
fn main() {
    ...
    // POPRAWKA 4: file_system() jest funkcją, nie makrem
    file_system();
    ...
    map.insert("alpha", 1); map.insert("beta", 2); map.insert("gamma", 3); map.insert("beta", 99);
