...
fn tick(_id: u8) { unsafe { TICK_COUNT += 1; } }
...
fn draw(_id: u8, ctx: &DrawCtx) {
    ...
    if unsafe { LAST_MSG_LEN } > 0 {
        let msg = unsafe { core::str::from_utf8(&LAST_MSG[..LAST_MSG_LEN]).unwrap_or("") };
...
fn on_cmd(_id: u8, _cmd_idx: u8, args: &str) {
    ...
    unsafe { LAST_MSG[..len].copy_from_slice(&b[..len]); LAST_MSG_LEN = len; }
}

define_plugin! {
    export_as: HELLO_PLUGIN,
    name:      "hello",
    version:   (0, 1, 0),
