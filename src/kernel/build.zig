...
pub fn build(b: *std.Build) void {
    ...
    // -----------------------------------------------------------------------
    // Ada — allocator integrity / audit / lifecycle (existing)
    // -----------------------------------------------------------------------
    const ada_flags = "-fno-exceptions -fno-stack-protector -O2 -mno-red-zone -mcmodel=large";
    const ada_out = "../../../../../build";
    const ada_src = "src/allocator/ada";
    const sec_src = "src/root_file_system/ada";
    ...
    const ada_stubs = b.addSystemCommand(&.{
        "cc",            "-c",                        "-O2",
        "-mno-red-zone", "-mcmodel=large",            ada_src ++ "/gnat_runtime_stubs.c",
        "-o",            build_dir ++ "/ada_stubs.o",
    });

    // -----------------------------------------------------------------------
    // Ada — disk security layer (new)
    // -----------------------------------------------------------------------
    const ada_disksec = b.addSystemCommand(&.{
        "sh", "-c",
        "cd " ++ sec_src ++ " && " ++
            "gnat compile " ++ ada_flags ++ " disksecurity.adb && " ++
            "mv disksecurity.o " ++ ada_out ++ "/ada_disksecurity.o",
    });
    const ada_diskauth = b.addSystemCommand(&.{
        "sh", "-c",
        "cd " ++ sec_src ++ " && " ++
            "gnat compile " ++ ada_flags ++ " diskauth.adb && " ++
            "mv diskauth.o " ++ ada_out ++ "/ada_diskauth.o",
    });
    const ada_changemon = b.addSystemCommand(&.{
        "sh", "-c",
        "cd " ++ sec_src ++ " && " ++
            "gnat compile " ++ ada_flags ++ " changemonitor.adb && " ++
            "mv changemonitor.o " ++ ada_out ++ "/ada_changemonitor.o",
    });
    const ada_cryptofs = b.addSystemCommand(&.{
        "sh", "-c",
        "cd " ++ sec_src ++ " && " ++
            "gnat compile " ++ ada_flags ++ " cryptofs.adb && " ++
            "mv cryptofs.o " ++ ada_out ++ "/ada_cryptofs.o",
    });

    // -----------------------------------------------------------------------
    // C glue layer
    // -----------------------------------------------------------------------
    const glue_c = b.addSystemCommand(&.{
        "cc",                   "-c",              "-O2",
        "-mno-red-zone",        "-mcmodel=large",  "-ffreestanding",
        "-fno-stack-protector", "-fno-exceptions", "-nostdinc",
        sec_src ++ "/glue.c",   "-o",              build_dir ++ "/glue.o",
    });
    ...
    cargo_kernel.step.dependOn(&ada_disksec.step);
    cargo_kernel.step.dependOn(&ada_diskauth.step);
    cargo_kernel.step.dependOn(&ada_changemon.step);
    cargo_kernel.step.dependOn(&ada_cryptofs.step);
    cargo_kernel.step.dependOn(&glue_c.step);
    ...
    const link_kernel = b.addSystemCommand(&.{
        ...
        // Security Ada + glue
        build_dir ++ "/ada_disksecurity.o",
        build_dir ++ "/ada_diskauth.o",
        build_dir ++ "/ada_changemonitor.o",
        build_dir ++ "/ada_cryptofs.o",
        build_dir ++ "/glue.o",
        // Rust kernel
    ...
    // Clean
    // -----------------------------------------------------------------------
    const clean = b.step("clean", "Clean kernel build artifacts");
    const clean_cmd = b.addSystemCommand(&.{
        ...
        build_dir ++ "/ada_stubs.o",       build_dir ++ "/ada_disksecurity.o",
        build_dir ++ "/ada_diskauth.o",    build_dir ++ "/ada_changemonitor.o",
        build_dir ++ "/ada_cryptofs.o",    build_dir ++ "/glue.o",
