...
unsafe fn goc(tab: PhysAddr, idx: usize, flags: u64) -> PhysAddr {
    ...
    // Merge in missing flags (e.g. PTE_U for user walk)
...
/// Unmap a 4K page. If the PT becomes empty, free it.
...
pub unsafe fn virt_to_phys(p4: PhysAddr, v: VirtAddr) -> Option<PhysAddr> {
    ...
    macro_rules! walk {
        ($tab:expr, $idx:expr) => {{
            ...
            if pte_huge(e) {
                // Assume max 2 MB huge pages in PD
...
/// User half (PML4[256..511]) cloned with CoW: every user leaf PTE
        ...
        for i3 in 0..512 {
            ...
            for i2 in 0..512 {
                ...
                for i1 in 0..512 {
                    ...
                    // Mark CoW: strip PTE_W, set PTE_COW in both parent and child
...
/// Returns true if resolved (retry), false if genuine access violation.
pub unsafe fn handle_cow_fault(p4: PhysAddr, fault_addr: VirtAddr) -> bool {
    ...
    if !frame_shared(old_phys) {
        ...
        MM_LOCK.unlock(); // drop lock around allocation
