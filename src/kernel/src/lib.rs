...
unsafe fn set_free(idx: usize) {
    ...
    if idx/64 < ALLOC_HINT { ALLOC_HINT = idx/64; }
}
...
unsafe fn init_gdt() {
    ...
    GDT.entries[1] = GdtE::new(0, 0xFFFFF, 0x9A, 0x20);
    GDT.entries[2] = GdtE::new(0, 0xFFFFF, 0x92, 0x00);
    GDT.entries[3] = GdtE::new(0, 0xFFFFF, 0xFA, 0x20);
    GDT.entries[4] = GdtE::new(0, 0xFFFFF, 0xF2, 0x00);
    ...
    // Daleki skok żeby załadować CS=0x08, potem załaduj pozostałe segmenty
    asm!(
        "push 0x08",
        "lea rax, [rip + 2f]",
        "push rax",
...
unsafe fn init_idt() {
    IDT[0x08] = IdtE::new(isr_double_fault as *const () as u64, 0x08, 0, 1);
    IDT[0x0E] = IdtE::new(isr_page_fault   as *const () as u64, 0x08, 0, 0);
    IDT[0x20] = IdtE::new(isr_timer        as *const () as u64, 0x08, 0, 0);
    IDT[0x80] = IdtE::new(isr_syscall      as *const () as u64, 0x08, 3, 0);

    IDTR.limit = (core::mem::size_of::<[IdtE; IDT_LEN]>() - 1) as u16;
...
// ============================================================================
// ISR MAKRA — Intel syntax w naked_asm!
// ============================================================================
...
pub unsafe fn thread_init() {
    print("[THREAD] Init\n");
    let tid = thread_create_kernel("idle\0", kernel_idle as *const () as VirtAddr, 0);
    if tid >= 0 {
...
unsafe fn thread_create_impl(name: &str, entry: VirtAddr, arg: u64, user: bool) -> i32 {
    for i in 0..MAX_THREADS {
        ...
        for p in 0..(KERNEL_STACK_SIZE / PAGE_SIZE) {
            ...
            mm_map_page(KERNEL_P4_PHYS, virt, phys, PTE_WRITABLE);
        }
        let k_top = k_start + KERNEL_STACK_SIZE as u64;

        let (u_top, cr3) = if user {
            let new_cr3 = create_user_p4();
        ...
        push!(if user { trampoline_user as *const () as u64 } else { trampoline_kernel as *const () as u64 });
        push!(0u64);
        push!(0u64);
        push!(0u64);
        push!(u_top);
...
pub extern "C" fn kernel_main() -> ! {
    unsafe {
        ...
        for i in 0..3u64 {
            thread_create_kernel("worker\0", test_thread as *const () as VirtAddr, i);
        }
